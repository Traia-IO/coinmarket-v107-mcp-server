# Coinmarket v107 MCP Server Implementation Guide

You are working on implementing the Coinmarket v107 MCP (Model Context Protocol) server. The basic structure has been set up, and your task is to implement the actual API integration.

## API Information

- **API Name**: Coinmarket v107
- **Documentation**: []()
- **Website**: [https://pro-api.coinmarketcap.com/](https://pro-api.coinmarketcap.com/)
- **Authentication**: Required - API key via `COINMARKET_V107_API_KEY` environment variable
...
When a client provides their own Coinmarket v107 API key:
...
Flow:
  1. Client connects with their Coinmarket v107 API key
  2. MCP server uses client's API key to call Coinmarket v107 API
  3. No payment required
  4. Client pays Coinmarket v107 directly (not the MCP server)
```

**Use Case**: Clients who already have a Coinmarket v107 subscription/API key
...
Flow:
  ...
  4. MCP server uses its INTERNAL Coinmarket v107 API key to call the API
  5. Client pays the MCP server (not Coinmarket v107)
```

**Use Case**: Pay-per-use clients without their own Coinmarket v107 subscription
...
async def your_tool_name(context: Context, param1: str) -> Dict[str, Any]:
    ...
    if api_key:
        ...
        api_key_to_use = os.getenv("COINMARKET_V107_API_KEY")
...
**Required**:
- `COINMARKET_V107_API_KEY`: Server's internal Coinmarket v107 API key (used when clients pay via 402)
...
COINMARKET_V107_API_KEY=your_coinmarket-v107_api_key_here
...
```json
{
  "mcp_server": {
    "capabilities": [
      ...
      "search_coinmarket_v107",
      "get_coinmarket_v107_info",
      // Add all other tools you implement
    ]
  },
  "tags": ["coinmarket v107", "api", /* add relevant tags like "search", "data", etc. */]
}
```
...
Add functions to call the Coinmarket v107 API with retry support. Example pattern:
...
def call_coinmarket_v107_api(endpoint: str, params: Dict[str, Any], api_key: str) -> Dict[str, Any]:
    """Call Coinmarket v107 API endpoint with automatic retry."""
...
"Content-Type": "application/json"
    ...
    try:
        ...
        raise Exception(f"Coinmarket v107 API error: {str(e)}")
```
...
```python
@mcp.tool()
async def search_coinmarket_v107(
    context: Context,
    query: str,
    limit: int = 10
) -> Dict[str, Any]:
    """
    Search Coinmarket v107 for [specific data type].
    ...
    try:
        # The call_coinmarket_v107_api function already has retry logic
        results = call_coinmarket_v107_api(
...
```python
@mcp.tool()
async def get_coinmarket_v107_info(
    context: Context,
    item_id: str
) -> Dict[str, Any]:
    """
...
```python
@mcp.tool()
async def create_coinmarket_v107_item(
    context: Context,
    name: str,
    data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    Create a new item in Coinmarket v107.
...
3. **Add Tool Examples** in README.md:
   ...
   result = await tool.search_coinmarket_v107(query="example", limit=5)
   ```
...
### 9. Common Coinmarket v107 Use Cases

Based on the API documentation, consider implementing tools for these common use cases:

1. **TODO**: List specific use cases based on Coinmarket v107 capabilities
2. **TODO**: Add more relevant use cases
3. **TODO**: Include any special features of this API

### 10. Example API Calls

Here are some example API calls from the Coinmarket v107 documentation that you should implement:
...
- Check the Coinmarket v107 API documentation: 
- Review the MCP specification: https://modelcontextprotocol.io
- Look at other MCP server examples in the Traia-IO organization

Remember: The goal is to make Coinmarket v107's capabilities easily accessible to AI agents through standardized MCP tools. 