...
# Script to build and run the Coinmarket v107 MCP Server locally in Docker
...
IMAGE_NAME="coinmarket-v107-mcp-server"
CONTAINER_NAME="coinmarket-v107-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running Coinmarket v107 MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# Coinmarket v107 MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO

# API Authentication (server's internal key for payment mode)
COINMARKET_V107_API_KEY=
...
if ! grep -q "^COINMARKET_V107_API_KEY=[^[:space:]]" .env; then
    if [ -t 0 ]; then
        echo ""
        echo -e "${YELLOW}📝 Please enter your Coinmarket v107 API key:${NC}"
        echo -e "${YELLOW}   (This is the server's internal API key used when clients pay via 402)${NC}"
        read -p "COINMARKET_V107_API_KEY: " API_KEY_VALUE
        if [ -n "$API_KEY_VALUE" ]; then
            if grep -q "^COINMARKET_V107_API_KEY=" .env; then
                if [[ "$OSTYPE" == "darwin"* ]]; then
                    sed -i '' "s|^COINMARKET_V107_API_KEY=.*|COINMARKET_V107_API_KEY=$API_KEY_VALUE|" .env
                else
                    sed -i "s|^COINMARKET_V107_API_KEY=.*|COINMARKET_V107_API_KEY=$API_KEY_VALUE|" .env
                fi
            else
                echo "COINMARKET_V107_API_KEY=$API_KEY_VALUE" >> .env
            fi
        ...
        echo -e "${YELLOW}⚠️  Non-interactive mode: Please set COINMARKET_V107_API_KEY in .env manually${NC}"
    fi
else
    echo -e "${GREEN}✅ COINMARKET_V107_API_KEY already set in .env${NC}"
fi
...
echo
echo -e "${GREEN}✅ Coinmarket v107 MCP Server is running!${NC}"
echo
