# Coinmarket v107 MCP Server

This is an MCP (Model Context Protocol) server that provides with authentication via Bearer tokens access to the Coinmarket v107 API. It enables AI agents and LLMs to interact with Coinmarket v107 through standardized tools.

## Features

- 🔧 **MCP Protocol**: Built on the Model Context Protocol for seamless AI integration
- 🌐 **Full API Access**: Provides tools for interacting with Coinmarket v107 endpoints
...
- **Coinmarket v107 Website**: [https://pro-api.coinmarketcap.com/](https://pro-api.coinmarketcap.com/)
- **API Documentation**: []()
...
*Note: Replace `example_tool` with actual Coinmarket v107 API tools based on the documentation.*
...
1. Clone this repository:
   ```bash
   git clone https://github.com/Traia-IO/coinmarket-v107-mcp-server.git
   cd coinmarket-v107-mcp-server
   ```

2. Set your API key:
   ```bash
   export COINMARKET_V107_API_KEY="your-api-key-here"
   ```
...
# Server's internal API key (for payment mode)
   COINMARKET_V107_API_KEY=your-api-key-here
...
2. Run the server:
   ```bash
COINMARKET_V107_API_KEY="your-api-key-here" uv run python -m server
   ```
...
Clients with their own Coinmarket v107 API key can use the server for free:

```bash
# Request with client's API key
curl -X POST http://localhost:8000/mcp \
  -H "Authorization: Bearer CLIENT_COINMARKET_V107_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"method":"tools/call","params":{"name":"example_tool","arguments":{"query":"test"}}}'
```

**Flow**:
1. Client provides their Coinmarket v107 API key
2. Server uses client's API key to call Coinmarket v107 API
3. No payment required
4. Client pays Coinmarket v107 directly
...
6. Server uses its INTERNAL Coinmarket v107 API key to call the API
...
```bash
# Required
COINMARKET_V107_API_KEY=your_internal_coinmarket-v107_api_key  # Server's API key (for payment mode)
...
1. **Client Decision**:
   - Has Coinmarket v107 API key? → Mode 1 (Authenticated)
...
1. Create API client functions for Coinmarket v107 endpoints
...
```json
{
  "github_url": "https://github.com/Traia-IO/coinmarket-v107-mcp-server",
  "mcp_server": {
    "name": "coinmarket-v107-mcp",
    "description": "Another mcp to test icons",
...
- `COINMARKET_V107_API_KEY`: Your Coinmarket v107 API key (required)
## Troubleshooting
