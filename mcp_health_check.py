#!/usr/bin/env python
"""
Coinmarket v107 MCP Server Health Check Script

This script properly connects to the Coinmarket v107 MCP server and checks its health by:
...
def check_mcp_server_health(url: str) -> bool:
    """Check if Coinmarket v107 MCP server is healthy"""
    print(f"🔍 Checking Coinmarket v107 MCP server health at {url}")
    ...
    if "error" in result and "session" not in str(result.get("error", "")):
        ...
        if "result" in result and "tools" in result["result"]:
            ...
            if missing_tools:
                ...
                print("✅ All expected Coinmarket v107 tools are available!")
...
def main():
    parser = argparse.ArgumentParser(description="Check Coinmarket v107 MCP Server Health")
    parser.add_argument("--url", default="http://localhost:8000", 
                       help="Coinmarket v107 MCP server URL (default: http://localhost:8000)")
    args = parser.parse_args()
    
    print(f"🚀 Coinmarket v107 MCP Server Health Check")
    print(f"📍 Server URL: {args.url}")
    print(f"📰 Expected tools: example_tool")
    print("="*50)
    
    if check_mcp_server_health(args.url):
        print("\n✅ Coinmarket v107 MCP Server is healthy and responding!")
        return 0
    else:
        print("\n❌ Coinmarket v107 MCP Server health check failed!")
        return 1
